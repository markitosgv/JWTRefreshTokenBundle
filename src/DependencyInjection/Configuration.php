            ...
            ->children()
                ->integerNode('ttl')
                    ...
                    ->validate()
                        ...
                        ->thenInvalid('The "ttl" must be a positive number of seconds, and %s is not. A token issued with it has already expired by the time it is handed over, so every refresh made with one fails. There is no value meaning never: use a large number of seconds instead.')
                    ->end()
                ->end()
                ->integerNode('max_tokens_per_user')
                    ...
                    ->validate()
                        ->ifTrue(static fn (?int $v): bool => null !== $v && $v < 1)
                        ->thenInvalid('The "max_tokens_per_user" must be at least 1, and %s is not. A limit of zero would revoke the token of the login that just succeeded.')
                    ->end()
                ->end()
