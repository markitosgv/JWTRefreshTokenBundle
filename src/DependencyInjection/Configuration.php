    ...
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $rootNode
            ->children()
                ...
                ->end()
                ->arrayNode('block_jwts_on_revocation')
                    ->canBeEnabled()
                    ->info('Refuses the JWTs already issued to a user when their refresh tokens are revoked with revokeAllForUser(). Off by default. Without it a password reset takes the refresh tokens away and leaves every JWT working until it expires.')
                    ->children()
                        ->scalarNode('cache')
                            ->defaultValue('cache.app')
                            ->cannotBeEmpty()
                            ->info('The PSR-6 pool the revocation marks are kept in. Read on every authenticated request, so a fast one, and shared between your processes: a local pool leaves a revoked user signed in wherever the mark did not reach.')
                        ->end()
                        ->integerNode('ttl')
                            ->defaultValue(3600)
                            ->min(1)
                            ->info('How long a revocation mark is kept, in seconds. It only has to outlive the JWTs issued before it, so this must be at least your lexik_jwt_authentication.token_ttl. Shorter and the oldest of those tokens start being accepted again.')
                        ->end()
                        ->scalarNode('user_claim')
                            ->defaultValue('username')
                            ->cannotBeEmpty()
                            ->info('The payload claim carrying the user, which is Lexik\'s "user_id_claim". A payload without it is left to the rest of the verification rather than refused.')
                        ->end()
                    ->end()
                ->end()
